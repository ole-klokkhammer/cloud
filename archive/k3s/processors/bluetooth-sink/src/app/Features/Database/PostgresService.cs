...
public class PostgresService : IHostedService
{
    ...
    public async Task InsertDataAsync(string command, Dictionary<string, object> data)
    {
        if (dataSource == null)
        {
            logger.LogError("Data source is not initialized. Call Initialize() before using this method.");
            return;
        }

        logger.LogDebug($"Executing command: {command} with parameters: {string.Join(", ", data.Select(kvp => $"{kvp.Key}={kvp.Value}"))}");
        await using var conn = dataSource.CreateConnection();
        await conn.OpenAsync();
