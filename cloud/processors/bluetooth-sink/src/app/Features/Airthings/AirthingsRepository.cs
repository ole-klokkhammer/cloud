...
public class AirthingsRepository
{
    ...
    public async Task InsertBatteryDataAsync(AirthingsBatteryData airthingsBatteryData, int serial)
    {
    ...
    public async Task InsertSensorDataAsync(AirthingsSensorData sensorData, int serial)
    {
        await postgresService.InsertDataAsync(
            @$"  
    ...
    public async Task InsertDeviceAsync(AirthingsDevice airthingsDevice, string mac, string? locationId = null)
    {
        await postgresService.InsertDataAsync(
            @$"
                INSERT INTO bluetooth.airthings_device (
                    mac, serial, model_number, manufacturer_name, hardware_revision, firmware_revision, location_id
                ) VALUES (
                    @mac, @serial, @model_number, @manufacturer_name, @hardware_revision, @firmware_revision, @location_id
                );
            ",
            new Dictionary<string, object>
            {
                ...
                { "location_id", locationId ?? (object)DBNull.Value }
            }
        );
    }
}