...
The "ai-utils" role inside the camagent LXC: embeds the detector's stills
(CLIP ViT-B/32, fp16 on the 5060 Ti) into postgres
(`events.embedding`, pgvector) and publishes one NATS message
per still. Also serves `POST /embed {text}` so the query agent can embed
search text into the same space.

- polls `/detector/events` (read-only mount) every 5s
