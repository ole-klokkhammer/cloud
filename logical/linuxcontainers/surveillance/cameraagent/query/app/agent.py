...
calls and feeds results back:
  ...
  count_events     - SQL count over events
...
LLM_BASE = env("QUERY_LLM_BASE_URL", "http://llm.homelan:8000/v1").rstrip("/")
LLM_MODEL = env("QUERY_LLM_MODEL", "qwen38-nvfp4")
...
def count_events(cls=None, camera=None, since_hours=None):
    q, params = "SELECT count(*) AS n FROM events", []
    if cls:
        q += " WHERE class = %s"
        params.append(cls)
    if camera:
    ...
    if since_hours:
        q += (
            " AND" if (cls or camera) else " WHERE"
        ) + " ts >= now() - (%s || ' hours')::interval"
        params.append(str(since_hours))
    return sql(q, params)[0]["n"]


def recent_events(cls=None, camera=None, limit=20):
    q, params = ("SELECT ts, camera, class, conf, still_path " "FROM events"), []
    if cls:
        q += " WHERE class = %s"
        params.append(cls)
    if camera:
...
def get_clip(camera, start, duration=60):
    url = (
        f"{MEDIAMTX}/get?path={camera}&start={start}"
...
SYSTEM = (
    "You answer questions about a home camera system. Use the tools for facts "
