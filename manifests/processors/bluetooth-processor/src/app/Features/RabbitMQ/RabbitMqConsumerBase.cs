...
public abstract class RabbitMqProcessorBase<TConsumer> : BackgroundService
{
    ...
    protected RabbitMqProcessorBase(
        JsonUtil json,
    ...
    protected override async Task ExecuteAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    await OnMessage(channel, model, ea);
    ...
    protected abstract Task OnMessage(IChannel channel, object model, BasicDeliverEventArgs ea);
