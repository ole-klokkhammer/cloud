...
class MotionDetector:
    ...
    def run(self):
        try:
            ...
            processing_fps = 15.0
            processing_frame_interval = 1.0 / processing_fps
            last_processed_time = time.time()

            # bg_sub = cv2.createBackgroundSubtractorMOG2(history=100, varThreshold=25)

            while True:
                current_time = time.time()
                if current_time - last_processed_time < processing_frame_interval:
                    continue
                last_processed_time = time.time()
                ...
                # if cv2.countNonZero(fg_mask) > 10000:
                #     logging.info(f"Motion detected in foreground mask: {cv2.countNonZero(fg_mask)}") 
    ...
    def simplify_frame(self, frame, size=(1024, 768), blur_kernel=(51, 51)):
        """
    ...
    def get_contours(
        ...
        """
        frameDelta = cv2.absdiff(reference_frame, simplified_frame)
        _, thresh_frame = cv2.threshold(
            frameDelta, threshold_value, 255, cv2.THRESH_BINARY
        )
        if use_morph_open:
